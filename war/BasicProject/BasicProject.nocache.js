...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "BasicProject" because\n';
  ...
  $wnd.__gwt_activeModules['BasicProject'] = {
 ...
 * "/BasicProject.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['BasicProject'].superdevmode = true;

  var expectedSuffix = '/BasicProject.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + BasicProject + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:BasicProject';
  var devModeSessionKey = '__gwtDevModeSession:BasicProject';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/BasicProject/BasicProject.nocache.js';
  ...
  $wnd.__gwt_activeModules['BasicProject'].canRedirect = true;
  $wnd.__gwt_activeModules['BasicProject'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING BasicProject';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/BasicProject?user.agent=' + ua + '&_callback=' + callback;
